...
export function AccountSurveyDetailModal({
    ...
    // 📅 기간 조회를 위한 연도 상태 관리 (기본값: 현재 연도 2026년)
    ...
    const availableYears = React.useMemo(() => {
        ...
        // 중복 제거 후 오름차순 정렬 (예: [2024, 2025, 2026])
        return Array.from(new Set(years)).sort((a, b) => a - b);
    }, [details, currentYear]);

    // 2. 선택된 연도(selectedYear)에 해당하는 데이터만 필터링
        ...
        <Dialog open={open} onOpenChange={(val) => !val && onClose()}>
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="mt-4 space-y-4">
                    ...
                    {/* ⏱️ 상단 연도 필터링 버튼 영역 (기존 UI 스타일 완벽 복원) */}
                                        ...
                                        {filteredItems.length === 0 && (
                                            <tr>
                                                <td colSpan={5} className="py-12 text-center text-sm text-gray-400 bg-white">
                                                    {selectedYear}년도에 조회된 답사 성과 데이터가 없습니다.
                                                </td>
                                            </tr>
                                        )}
                                    </tbody>
